...
class App:
    def __init__(self, root):
        ...
        self.task_manager_button = tk.Button(root, text="Task Manager", command=self.show_task_manager)
        self.task_manager_button.place(x=260, y=170, width=97, height=30)

        # Button for Event Manager
        self.event_manager_button = tk.Button(root, text="Event Manager", command=self.show_event_manager)
        self.event_manager_button.place(x=260, y=240, width=97, height=30)

        # Attribute to store instances of ReminderApp and EventManager
    ...
    def show_task_manager(self):
        ...
        if not self.reminder_app:
            self.reminder_app = ReminderApp(self.root)  # Use the stored reference to root

        self.reminder_app.show_elements()
        self.task_manager_button["state"] = tk.DISABLED
        self.event_manager_button["state"] = tk.NORMAL

    def show_event_manager(self):
        if self.reminder_app:
            self.reminder_app.destroy()

        if not self.event_manager:
            self.event_manager = EventManager(self.root)  # Use the stored reference to root

        self.event_manager.show_elements()
        self.event_manager_button["state"] = tk.DISABLED
        self.task_manager_button["state"] = tk.NORMAL
...
class EventManager:
    def __init__(self, window):
        ...
        # Add event button
        add_event_button = tk.Button(window, text="Add Event", command=self.add_event)
